...
When following the [guide](https://www.raspberrypi.org/learning/astro-pi-guide/) you will have written a sample program which scrolls text across the LED matrix. The program contains two crucial lines, which import the Astro Pi software and create an `ap` object which represents the Astro Pi board.
...
The Astro Pi has a set of environmental sensors for detecting the conditions around it. It can detect:
...
### Ideas
  ...
  - You could improve the dice program from earlier in this activity, so that shaking the Astro Pi triggers the dice roll.
...
Now that you've explored most of the features of the Astro Pi HAT, you could combine them to create a project. Here's an example reaction testing game, which could be used by the astronauts to test their reflexes.
